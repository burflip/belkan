...
Environment::Environment(ifstream &infile) {
	...
	int aldeano = 0;
	int princesa = random() % 2;
	int princeso = random() % 2;
	int leonardo = random() % 2;
	int bruja = random() % 2;
	int profe = random() % 2;

	RepartirPersonajes(aldeano, princesa, princeso, leonardo, bruja, profe, 1);
	...
	for (int row = 0; row < tama; row += 1) {
		for (int col = 0; col < tama; col += 1) {
			...
			case 'a':
				switch (agentOriented_) {
				case 0:
					PintaQuesitoParaAbajo(x, y, size, 1);
					break;
				case 1:
					PintaQuesitoParaIzquierda(x, y, size, 1);
					break;
				case 2:
					PintaQuesitoParaArriba(x, y, size, 1);
					break;
				case 3:
					PintaQuesitoParaDerecha(x, y, size, 1);
					break;
				}
				break;
			case 'b':
...
void Environment::RepartirObjetos(int banador, int piedra, int espada, int rosa, int oro, int oscar, int zapatillas, int pala, int manzana,
		int algoritmo) {
...
void Environment::RepartirPersonajes(int aldeano, int Princesa, int princeso, int Leonardo, int bruja, int profe, int oso) {
	...
	if (golpeado_) {
		...
		case 9: // Lanzar
			if (en_uso_ == '-') {
				...
				switch (agentOriented_) {
				case 0:
					while (i < 5 and agentPosX_ - i >= 0
							and (maze_objetos_[agentPosX_ - i][agentPosY_] < 'a' or maze_objetos_[agentPosX_ - i][agentPosY_] > 'q')
							and maze_[agentPosX_ - i][agentPosY_] != 'B' and maze_[agentPosX_ - i][agentPosY_] != 'M'
							and maze_[agentPosX_ - i][agentPosY_] != 'D') {
					...
					break;
				case 1:
					while (i < 5 and agentPosY_ + i < 200
							and (maze_objetos_[agentPosX_][agentPosY_ + i] < 'a' or maze_objetos_[agentPosX_][agentPosY_ + i] > 'q')
							and maze_[agentPosX_][agentPosY_ + i] != 'B' and maze_[agentPosX_][agentPosY_ + i] != 'M'
							and maze_[agentPosX_][agentPosY_ + i] != 'D') {
					...
					break;
				case 2:
					while (i < 5 and agentPosX_ + i < 200
							and (maze_objetos_[agentPosX_ + i][agentPosY_] < 'a' or maze_objetos_[agentPosX_ + i][agentPosY_] > 'q')
							and maze_[agentPosX_ + i][agentPosY_] != 'B' and maze_[agentPosX_ + i][agentPosY_] != 'M'
							and maze_[agentPosX_ + i][agentPosY_] != 'D') {
				...
				if (alcanzado_) { // Has alcanzado algo
					cout << "fila: " << new_fila << "  columna: " << new_columna << " maze: " << maze_[new_fila][new_columna] << " " << "objetos: "
							<< maze_objetos_[new_fila][new_columna] << endl;
...
// ------------------------------------------------------------
void Environment::SenSorStatus(char *terreno, char *objetos, string &mensaje, bool &reiniciar, char &en_uso, char *mochila, char *puntuacion,
		bool &finjuego, char &salud, bool remote) {
	if (!remote)
...
bool Environment::Perceive_Remote(conexion_client &Cliente) {
	...
	Cliente.Recibir_Msg_NoBlocking(vista_, surface_, mensaje_, reiniciado_, en_uso_, mochila_, valor, finJuego_, salud, actualizado);
